...
static final String BASE_URL = "https://128.168.56.1/";
...
public void start() {
    ...
    public void onResponse(Call<List<Cliente>> call, Response<List<Cliente>> response) {
        if(response.isSuccessful()) {
            List<Cliente> changesList = response.body();
            for (int i=0;i<changesList.size();i++){
                Log.e("cliente", changesList.get(i).getDireccion());
            }
            Log.e("cliente", "sin conexion");
        } else {
